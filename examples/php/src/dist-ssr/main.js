...
function component_api_changed(parent2, method, component2) {
  if (DEV) {
    const error = new Error(`component_api_changed
${parent2} called \`${method}\` on an instance of ${component2}, which is no longer valid in Svelte 5. See https://svelte.dev/docs/svelte/v5-migration-guide#Components-are-no-longer-classes for more information`);
    error.name = "Svelte error";
    throw error;
  } else {
...
%c${parent2} passed a value to ${child2} with \`bind:\`, but the value is owned by ${owner}. Consider creating a binding between ${owner} and ${parent2}`, bold$1, normal$1);
  } else {
...
function add_owner_to_object(object, owner, seen2) {
  const metadata = (
    /** @type {ProxyMetadata} */
    ...
    if (ADD_OWNER in object && object[ADD_OWNER]) {
      ...
      if (proto === Object.prototype) {
        for (const key in object) {
          add_owner_to_object(object[key], owner, seen2);
        }
      } else if (proto === Array.prototype) {
        for (let i = 0; i < object.length; i += 1) {
          add_owner_to_object(object[i], owner, seen2);
        }
      }
    }
  }
...
function proxy(value, parent2 = null, prev) {
  var _a, _b;
  ...
  if (DEV) {
    ...
    if (prev) {
      ...
      metadata.owners = parent2 === null ? component_context !== null ? /* @__PURE__ */ new Set([component_context.function]) : null : /* @__PURE__ */ new Set();
    }
  }
  return new Proxy(
    /** @type {any} */
...
function get(signal) {
  ...
  signal.deps === null) {
    ...
    if (parent2 !== null && !((_a = parent2.deriveds) == null ? void 0 : _a.includes(derived2))) {
      (parent2.deriveds ?? (parent2.deriveds = [])).push(derived2);
    }
  }
  if (is_derived) {
...
function getContext$1(key) {
  const context_map = get_or_init_context_map$1("getContext");
  const result = (
    /** @type {T} */
    context_map.get(key)
  );
...
function setContext$1(key, context) {
  const context_map = get_or_init_context_map$1("setContext");
  context_map.set(key, context);
  return context;
}
function hasContext$1(key) {
  const context_map = get_or_init_context_map$1("hasContext");
  return context_map.has(key);
}
function getAllContexts$1() {
  const context_map = get_or_init_context_map$1("getAllContexts");
  if (DEV) {
...
function get_or_init_context_map$1(name) {
  if (component_context === null) {
    lifecycle_outside_component(name);
  }
  return component_context.c ?? (component_context.c = new Map(get_parent_context$1(component_context) || void 0));
}
function get_parent_context$1(component_context2) {
...
function mount$1(component2, options) {
  return _mount(component2, options);
}
function hydrate$1(component2, options) {
  init_operations();
  ...
  try {
    ...
    if (error === HYDRATION_ERROR) {
      ...
      set_hydrating(false);
      return mount$1(component2, options);
    }
    throw error;
  } finally {
...
function legacy_api() {
  ...
  function error(method) {
    var _a;
    const parent2 = ((_a = get_component()) == null ? void 0 : _a[FILENAME]) ?? "Something";
    component_api_changed(parent2, method, component2[FILENAME]);
  }
  return {
...
function inspect$1(get_value, inspector = console.log) {
...
const THEN = 1;
const CATCH = 2;
function await_block$1(node, get_input, pending_fn, then_fn, catch_fn) {
  if (hydrating) {
    hydrate_next();
  }
  var anchor = node;
...
function css_props$1(element2, get_styles) {
  if (hydrating) {
    set_hydrate_node(
      /** @type {TemplateNode} */
...
function html$1(node, get_value, svg, mathml, skip_warning) {
  var anchor = node;
  var value = "";
  var effect2;
  block(() => {
...
function slot$1(anchor, $$props, name, slot_props, fallback_fn) {
  var _a;
  if (hydrating) {
    hydrate_next();
  }
...
function sanitize_slots$1(props) {
  const sanitized = {};
...
function createRawSnippet$1(fn) {
...
const raf = {
  ...
  tick: (
    /** @param {any} _ */
    (_) => (DEV ? requestAnimationFrame : noop)(_)
  ),
  now: () => now(),
...
function store_get$1(store, store_name, stores) {
...
function store_set$1(store, value) {
  store.set(value);
  return value;
}
function invalidate_store(stores, store_name) {
  ...
  if (entry.store !== null) {
    store_set$1(entry.store, entry.source.v);
  }
}
...
function store_mutate$1(store, expression, new_value) {
  store.set(new_value);
  return expression;
}
function update_store$1(store, store_value, d = 1) {
...
// @__NO_SIDE_EFFECTS__
function rest_props$1(props, exclude, name) {
  return new Proxy(
...
class Svelte4Component {
  ...
  constructor(options) {
    ...
    __privateSet(this, _instance, (options.hydrate ? hydrate$1 : mount$1)(options.component, {
    ...
    __privateGet(this, _instance).$destroy = () => {
      unmount$1(__privateGet(this, _instance));
    };
  }
...
let parent = null;
let seen;
function stringify$1(element2) {
  if (element2.filename === null) return `\`<${element2.tag}>\``;
  return `\`<${element2.tag}>\` (${element2.filename}:${element2.line}:${element2.column})`;
}
function print_error(payload, parent2, child2) {
  var message = (parent2 === null ? `node_invalid_placement_ssr: ${stringify$1(child2)} needs a valid parent element

` : `node_invalid_placement_ssr: ${stringify$1(parent2)} cannot contain ${stringify$1(child2)}
...
function push_element(payload, tag, line, column) {
  ...
  if (parent !== null) {
    ...
    if (!is_tag_valid_with_parent(tag, parent.tag)) {
      print_error(payload, parent, child2);
    }
    while (ancestor != null) {
      ancestors.push(ancestor.tag);
      if (!is_tag_valid_with_ancestor(tag, ancestors)) {
        print_error(payload, ancestor, child2);
      }
      ancestor = ancestor.parent;
    }
  } else if (!is_tag_valid_with_parent(tag, null)) {
    print_error(payload, null, child2);
  }
  parent = child2;
}
...
function css_props(payload, is_html, props, component2, dynamic = false) {
...
if (typeof window === "undefined") {
  const modules = /* @__PURE__ */ Object.assign({ "./lib/Counter.svelte": () => import("./lib/Counter.js"), "./lib/Counter2.svelte": () => import("./lib/Counter2.js"), "./lib/child/Counter.svelte": () => import("./lib/child/Counter.js") });
